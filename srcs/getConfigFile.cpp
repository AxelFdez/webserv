...
/*   Updated: 2023/11/14 10:30:05 by axfernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
getConfigFile::getConfigFile( char* file ) {
    ...
    if ( argFile.is_open() ) {
        ...
        if ( _serverToken == true ) {
            throw std::invalid_argument( RED "Error:\nconfig file: syntax error: ']' missing! line: " + std::to_string( lineNumber )+ " getFile 3" ); RESET;
        }
    }
...
void getConfigFile::checkLocationScope( std::string line, size_t lineNumber ) {
    ...
    if ( line.find( "location", 0, 8 ) == 0 && line.at(line.size() -1) == '{' &&  _locationToken == true ) {
        throw std::invalid_argument( RED "Error:\nconfig file: syntax error: '" + line + "' line: " + std::to_string( lineNumber )+ " checkLocationScope 1" ); RESET;
    }
    ...
    if ( line == "}" &&  _locationToken == true ) {
        ...
        throw std::invalid_argument( RED "Error:\nconfig file: syntax error: '" + line + "' line: " + std::to_string( lineNumber -1 )+ " checkLocationScope 2" ); RESET;
    }
}
...
void getConfigFile::checkSyntax_server_location( std::string & line, size_t lineNumber ) {
    ...
    if ( line.find( "server", 0, 5 ) == 0 && line.at(line.size() -1) != ';' ) {
        ...
        if ( line.at(line.size()-1) != '[' ) {
            throw std::invalid_argument( RED "Error:\nconfig file: syntax error: '[' missing! line: " + std::to_string( lineNumber )+ " check check Syntax 1" ); RESET;
        }
        for ( size_t i = 6; i != line.size() -1; i++ ) {
            if ( !isspace(line[i]) ) {
                throw std::invalid_argument( RED "Error:\nconfig file: syntax error: " + line + " line: " + std::to_string( lineNumber )+ " check check Syntax 2" ); RESET;
            }
        }
    ...
    if ( line.find( "location", 0, 8 ) == 0 && line.at(line.size() -1) != ';' ) {
        if ( line.at(line.size()-1) != '{' ) {
            throw std::invalid_argument( RED "Error:\nconfig file: syntax error: '{' missing! line: " + std::to_string( lineNumber )+ " check check Syntax 3" ); RESET;
        }
    }
}
...
void getConfigFile::checkSemicolon( std::string const& line, size_t lineNumber ) const{
    ...
    if (  (line.find("]") != std::string::npos || line.find("}") != std::string::npos) ) {
        ...
        if ( (line.size() != 1) ){
            throw std::invalid_argument( RED "Error:\nconfig file: syntax error: line: " + std::to_string( lineNumber ) + " check Semicolon 1" ); RESET;
        }
        return;
    }
    ...
    if ( line.find(";") != std::string::npos ) {
        throw std::invalid_argument( RED "Error:\nconfig file: syntax error: line: " + std::to_string( lineNumber ) + " check Semicolon 2" ); RESET;
    }
        return;
    }
    ...
    else { throw std::invalid_argument( RED "Error:\nconfig file: syntax error: line: " + std::to_string( lineNumber ) + " check Semicolon 3" ); RESET; }
}
...
void getConfigFile::checkValue( std::string line, size_t lineNumber ) {
    ...
    if ( line.find("index", 0, 5) == 0 && !isspace(line[5]) || line.find("methods", 0, 7) == 0 && !isspace(line[7]) ) {
        throw std::invalid_argument( RED "Error:\nconfig file: syntax error: line: chValue1 " + std::to_string( lineNumber ) ); RESET;
    }
    else if ( line != "server[" && line != "]" && line != "}" ) {

        if ( !(find_if( line.begin(), line.end(), ::isspace ) != line.end()) )   {
            throw std::invalid_argument( RED "Error:\nconfig file: syntax error: line: chValue2 " + std::to_string( lineNumber ) ); RESET;
        }
    }
    if ( line.find("download ", 0, 9) == 0 || line.find("directory_listing", 0, 17) == 0 ) {
        line = getValue( line );
        if (line != "on" && line != "off" ) {
            throw std::invalid_argument( RED "Error:\nconfig file: wrong value: must be on/off line: " + std::to_string( lineNumber ) ); RESET;
        }
    }
...
void getConfigFile::cleanLine( std::string & line, size_t lineNumber ) {
...
void getConfigFile::checkBodySize( std::string line, size_t lineNumber ) {
    ...
    if ( value.empty() ) {
        throw std::invalid_argument( RED "Error:\nconfig file: no value: 'body_size_max directive' line: " + std::to_string( lineNumber ) + " check listen" ); RESET;
    }
    else {
        ...
        if ( value.find_first_not_of("0123456789") == std::string::npos
            ...
            throw std::invalid_argument( RED "Error:\nconfig file: wrong value: 'body_size_max directive' line: " + std::to_string( lineNumber ) + " check listen" ); RESET;
        }
    }
}
    ...
    for ( size_t i = 0; i < line.length(); i++ ) {
        ...
        if ( isspace(line[i]) || i == line.length() -1) {
            ...
            if ( tmp != "GET" && tmp != "POST" && tmp != "DELETE" || methNumber > 3) {
                ...
                throw std::invalid_argument( RED "Error:\nconfig file: 'methods directive' line: " + std::to_string( lineNumber ) + " check Methods" ); RESET;
            }
...
void getConfigFile::checkListen( std::string line, size_t lineNumber ) {
    ...
    std::string value = getValue( line );
    if ( value.empty() ) {
        throw std::invalid_argument( RED "Error:\nconfig file: no value: 'listen directive' line: " + std::to_string( lineNumber ) + " check listen" ); RESET;
    }
    else {
        std::istringstream iss( value );
        int number;
        if ( value.find_first_not_of("0123456789") == std::string::npos
            ...
            throw std::invalid_argument( RED "Error:\nconfig file: wrong value: 'listen directive' line: " + std::to_string( lineNumber ) + " check listen" ); RESET;
        }
    }
}
...
void getConfigFile::checkHost( std::string line, size_t lineNumber ) {
    ...
    if ( std::count( value.begin(), value.end(), '.' ) != 3
        ...
        || value.find_first_not_of("0123456789.") != std::string::npos ) {
            throw std::invalid_argument( RED "Error:\nconfig file: wrong value: 'host directive' line: " + std::to_string( lineNumber ) + " check host1" ); RESET;
    }
    for ( int i = 0; i < 4; i++ ) {
        ...
        else { throw std::invalid_argument( RED "Error:\nconfig file: wrong value: 'host directive' line: " + std::to_string( lineNumber ) + " check host3" ); RESET; }
    }
}